...
Function Get-AADDatabricksToken{
    ...
    switch ($State) {
        ...
        "Expired" {
            ...
            $Body = "grant_type=refresh_token&client_id=$ClientId&refresh_token=$Refresh&resource=$resourceId"
        ...
        "Missing" {
            ...
            $BodyText="grant_type=client_credentials&client_id=$ClientId&resource=2ff814a6-3304-4ab8-85cb-cd0e6f879c1d&client_secret=$Secret"
...
Function Get-AADManagementToken{
    ...
    switch ($State) {
        ...
        "Missing" {
            ...
            $BodyText="grant_type=client_credentials&client_id=$ClientId&resource=https://management.core.windows.net&client_secret=$Secret"
            $Response = Invoke-RestMethod -Method POST -Body $BodyText -Uri $URI -ContentType application/x-www-form-urlencoded
        }
    }
